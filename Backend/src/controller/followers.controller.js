...
const recommendedUserFollowing = asyncHandler(async (req, res) => {
  const recommendedUsers = await UserDB.aggregate([
    {
      $lookup: {
        from: "followingdbs",
        localField: "_id",
    ...
    // },
    {
      $addFields: {
        ...
        unFollowedUser: {
          $cond: {
            if: {
              $in: [req.user?._id, "$followed.followedId"],
            },
            then: true,
            else: false,
          },
    ...
    {
      $match: {
        unFollowedUser: false,
      },
    },
    {
      $addFields: {
        unfollowedUserDetail: {
          username: "$username",
          avatar: "$avatar",
          coverImage: "$coverImage",
          isFollowed: "$unFollowedUser",
        },
      },
    },
    {
